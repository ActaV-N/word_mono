...
const NyamDetail:React.FC<Props> = ({nyam, showState}) => {
    const [filled, setFilled] = useState<boolean>(nyam.meaning && nyam.category && nyam.media ? true:false);

    return <div className='text-sm'>
        <div className='text-xs text-slate-600 text-right'>
            {nyam.createdAt.toLocaleString()}
        </div>
        ...
        {nyam.cases.length !== 0 && (
            <div className='px-3 my-7'>
                ...
                <div key={i} className='mb-3 flex items-start justify-between'>
                    <div>
                        <div className='text-xs text-slate-700'>
                            {useCase.createdAt.toLocaleString()}
                        </div>
                        <div>
                            <div className='py-1'>
                                {useCase.content}
                ...
                <div className='mt-3 text-xs text-slate-700 text-right'>
                    냠냠 완료까지 남은 경험: {3 - nyam.cases.length}
                </div>
            </div>    
        )}
        {nyam.media && 
        ...
        {showState === "zoop" && <NyamZoopDetail filled={filled} setFilled={setFilled} nyam={nyam} />}
